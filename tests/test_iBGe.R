...
## verify that the C engine and the R engine give numerically identical
## results

tol <- .Machine$double.eps^0.5

s_C  <- iBGe(g, dat, targets, target.index, engine="C")
s_R  <- iBGe(g, dat, targets, target.index, engine="R")
stopifnot(abs(s_C - s_R) < tol)

s2_C <- iBGe(g2, dat, targets, target.index, engine="C")
s2_R <- iBGe(g2, dat, targets, target.index, engine="R")
stopifnot(abs(s2_C - s2_R) < tol)

## pure observational data (no interventions)
sobs_C  <- iBGe(g,  dat, engine="C")
sobs_R  <- iBGe(g,  dat, engine="R")
stopifnot(abs(sobs_C - sobs_R) < tol)

sobs2_C <- iBGe(g2, dat, engine="C")
sobs2_R <- iBGe(g2, dat, engine="R")
stopifnot(abs(sobs2_C - sobs2_R) < tol)

## verify that cached.scores reuse works correctly through the (default)
## C engine, reusing the same cached.scores object across repeated calls,
## as is done by hcmc()/hillclimbing() during search

csco <- replicate(numNodes(g), new.env(hash=TRUE, parent=emptyenv()),
                  simplify=FALSE)

s_first  <- iBGe(g, dat, targets, target.index, cached.scores=csco)
s_second <- iBGe(g, dat, targets, target.index, cached.scores=csco)
s_third  <- iBGe(g, dat, targets, target.index, cached.scores=csco)
...
s2_cached <- iBGe(g2, dat, targets, target.index, cached.scores=csco)
stopifnot(abs(s2_cached - s2_C) < tol)

## verify cross-engine cache-key format parity: a cached.scores object
## populated by one engine must be readable, with identical scores, by the
## other engine

csco_R_populated <- replicate(numNodes(g), new.env(hash=TRUE, parent=emptyenv()),
                              simplify=FALSE)
s_via_R <- iBGe(g, dat, targets, target.index,
                cached.scores=csco_R_populated, engine="R")
s_via_C_cached <- iBGe(g, dat, targets, target.index,
...
csco_C_populated <- replicate(numNodes(g), new.env(hash=TRUE, parent=emptyenv()),
                              simplify=FALSE)
s_via_C <- iBGe(g, dat, targets, target.index,
               cached.scores=csco_C_populated, engine="C")
s_via_R_cached <- iBGe(g, dat, targets, target.index,
...
## exercising both the closed-form lp==0 fast path and the literal ":"
## cache-key special case, plus one child), and repeated cache hits/misses
...
s2n_C <- iBGe(g2n, dat2n, engine="C")
s2n_R <- iBGe(g2n, dat2n, engine="R")
stopifnot(abs(s2n_C - s2n_R) < tol)

csco_hitmiss <- replicate(numNodes(g), new.env(hash=TRUE, parent=emptyenv()),
                          simplify=FALSE)
s_allmiss <- iBGe(g, dat, targets, target.index,
                  cached.scores=csco_hitmiss, engine="C")
s_allhit  <- iBGe(g, dat, targets, target.index,
                  cached.scores=csco_hitmiss, engine="C")
stopifnot(abs(s_allmiss - s_allhit) < tol)

## no caching at all (cached.scores=NULL) through the C engine, repeated
s_nocache1 <- iBGe(g, dat, targets, target.index, cached.scores=NULL, engine="C")
s_nocache2 <- iBGe(g, dat, targets, target.index, cached.scores=NULL, engine="C")
